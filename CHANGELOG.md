...
- **VendorDetail restructured** — split into Overview/Compliance/Events/Notes tabs, and converted two oversized button-grid selectors (Data Sensitivity: 7 options, Review Schedule: 6 options) to dropdowns — a cognitive-load fix. (Initially shipped as native `<select>` elements for the accessibility win of not hand-rolling ARIA; corrected in a same-release follow-up below once the OS-rendered option popup turned out to visibly break the theme.)
...
**Follow-up fix, same release — native `<select>` popups broke the theme:** a native `<select>`'s option list is OS-rendered, not page-rendered — `colorScheme: 'dark'` nudges some browsers and does nothing on others, and no CSS reaches it. In practice this shipped a plain white/light system dropdown floating over the dark violet theme, screenshotted after the fact since Playwright can't capture native popup chrome (a real gap in the prior pass's own verification). Replaced with a hand-rolled `Select.jsx` — themed to match the app (same `var(--elevated)`/`var(--border)` shell as the Scan All tooltip and modals, same `cubic-bezier(0.16,1,0.3,1)` motion curve) and portaled to `document.body` rather than positioned inline, because `Panel`'s `animation: fade-up … both` leaves `transform: translateY(0)` applied after the animation ends, and any non-`none` ancestor transform creates a new containing block that breaks naively-positioned `position: fixed` children (the same reason `EventFeed`'s `AcceptedBadge` tooltip already portals). Rebuilding the accessibility the native element gave for free was the actual scope of the fix, not the styling: WAI-ARIA combobox pattern, focus stays on the trigger button throughout (tracked via `aria-activedescendant`, not by moving DOM focus into the list — the detail that most commonly breaks screen-reader announcement in a hand-rolled version of this), full keyboard support (arrows/Home/End/Enter/Escape/Tab/type-ahead), click-outside-to-close, active-option scroll-into-view. Verified via Playwright screenshots of the **open** popup state (a custom popup is real DOM, unlike the native one) at desktop and mobile, plus scripted keyboard navigation confirming `aria-activedescendant` and `aria-expanded` update correctly.
